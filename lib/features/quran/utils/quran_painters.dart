...
class QuranBgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRect(Rect.fromLTWH(0,0,size.width,size.height),
        Paint()..color = kNight);
    _s ??= List.generate(50, (_) =>
        Offset(_r.nextDouble()*size.width, _r.nextDouble()*size.height*0.5));
    for (int i = 0; i < _s!.length; i++) {
      final op = 0.03 + 0.1*((math.sin(t*2*math.pi+i)+1)/2);
      canvas.drawCircle(_s![i], 0.9, Paint()..color = kGold.withOpacity(op));
    }
    canvas.drawCircle(Offset(size.width/2, -40), 180,
        Paint()..shader = RadialGradient(
          colors: [kGold.withOpacity(0.05), Colors.transparent])
            .createShader(Rect.fromCircle(
                center: Offset(size.width/2, -40), radius: 180)));
    final p = Paint()..color = kGold.withOpacity(0.035)
  ...
  @override bool shouldRepaint(QuranBgPainter o) => o.t != t;
}

class SurahBadgePainter extends CustomPainter {
  final int n; final Color c;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(cx, r, Paint()
        ..color=c.withOpacity(0.3)..style=PaintingStyle.stroke..strokeWidth=1.5);
    for (int i=0; i<8; i++) {
      final a = i*math.pi/4;
      canvas.drawCircle(
        Offset(cx.dx+(r-3)*math.cos(a), cx.dy+(r-3)*math.sin(a)), 1.1,
        Paint()..color=c.withOpacity(0.35));
    }
    final tp = TextPainter(
      text: TextSpan(text: ar(n),
          style: GoogleFonts.amiri(fontSize: n>99?8:11, color: c, fontWeight: FontWeight.w700)),
      textDirection: TextDirection.rtl)..layout();
    tp.paint(canvas, Offset(cx.dx-tp.width/2, cx.dy-tp.height/2));
  }
  @override bool shouldRepaint(SurahBadgePainter o) => false;
}

class JuzRingPainter extends CustomPainter {
  final double progress; final Color c;
  JuzRingPainter(this.progress, this.c);
  @override
  void paint(Canvas canvas, Size size) {
    final cx = Offset(size.width/2, size.height/2), r = size.width/2-3;
    canvas.drawCircle(cx, r, Paint()
        ..color=kBorder..style=PaintingStyle.stroke..strokeWidth=3.5);
    if (progress <= 0) return;
    canvas.drawArc(Rect.fromCircle(center: cx, radius: r),
        -math.pi/2, 2*math.pi*progress, false,
        Paint()..color=c..style=PaintingStyle.stroke
            ..strokeWidth=3.5..strokeCap=StrokeCap.round);
  }
  @override bool shouldRepaint(JuzRingPainter o) => o.progress!=progress;
}
...
class VerseMarkerPaint extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(path, Paint()
        ..color=c.withOpacity(0.5)..style=PaintingStyle.stroke..strokeWidth=0.9);
    final tp = TextPainter(
      text: TextSpan(text: ar(n),
          style: GoogleFonts.amiri(fontSize: n>9?7:9, color: c, fontWeight: FontWeight.w700)),
      textDirection: TextDirection.rtl)..layout();
    tp.paint(canvas, Offset(cx.dx-tp.width/2, cx.dy-tp.height/2));
  }
  @override bool shouldRepaint(VerseMarkerPaint o) => o.n!=n;
}
...
class CornerDeco extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(const Offset(3, 3), 1.5,
        Paint()..color=c.withOpacity(0.5));
  }
  @override bool shouldRepaint(covariant CornerDeco o) => false;
}
